...
> **命名说明**：这一组测试在既有代码里的分组名是「无障碍性相关」，但它**实际测的是自定义属性透传**，不是无障碍功能。
>
> 本项目**不实现无障碍**（不写 ARIA、不做键盘导航、不设 `role` / `tabIndex`）。因此这一组只需验证：使用者传入的自定义属性（`data-*`、`name` 等）能正确落到 DOM 上。
>
> 部分组件另有**反向断言**用例，显式确认组件**不含** ARIA 属性——如 `src/tooltip/tests/Tooltip.test.ts` 的「不应该包含 ARIA 属性（项目规范）」。新组件建议保留这种断言，防止无意中引入。
