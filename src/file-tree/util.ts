...
export const formatBasicTree = (
  ...
  return trees.map((item) => {
    ...
    if (!(keyName in curItem) || !curItem[keyName as 'id']) {
...
export function useTreeNode(data: ComputedRef<IInnerTreeNode>): IUseTreeNode {
  const nodeClass = computed(() => [
    ns.e('node'),
    data.value?.expanded && ns.em('node', 'open'),
  ...
  const nodeVLineStyles = computed(() => {
    ...
    return Array.from({ length: data.value.level - 1 }).map((_, index) => ({
      height: `${
        currentIndex + 1 === parentChildNodeCount && index === 0
          ? isLeaf || !expanded
            ? NODE_HEIGHT / 2
            : NODE_HEIGHT
          : NODE_HEIGHT
...
export function generateInnerTree(
  ...
  return tree.reduce((acc: IInnerTreeNode[], item: ITreeNode, currentIndex) => {
    ...
    if (!newItem[key]) {
      ...
      return acc.concat(
        omit<ITreeNode>(newItem, 'children'),
        generateInnerTree(newItem[key], key, level, path),
      );
    }
  }, []);
}
