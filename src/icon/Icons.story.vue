...
const iconCategories = {
  ...
  搜索和缩放: ['search', 'zoom-in', 'zoom-out', 'zoom-to-fit', 'full-screen', 'view', 'hide'],
  状态图标: [
    'error',
    'warning',
    'info',
  ...
  媒体播放: ['play', 'pause', 'stop', 'next', 'previous', 'fast-forward', 'rewind'],
  用户和账户: [
    'contact',
    'group',
    'people',
  ...
  特殊图标: ['spinner-filled', 'plug-disconnected', 'plug-disconnected-filled'],
};
</script>
  ...
  <Story title="Basic/Icons">
    ...
    <Variant title="使用 TIcon 组件（通过 name 属性）">
      ...
      <div style="margin-top: 16px; font-size: 14px; color: #666">
        ✨ 通过
        <code style="background: #f5f5f5; padding: 2px 6px; border-radius: 4px">name</code>
        属性动态指定图标
      </div>
    </Variant>
        ...
        <div v-for="(icons, category) in iconCategories" :key="category">
          ...
          <div style="display: flex; gap: 12px; flex-wrap: wrap; align-items: center">
            ...
            >
              ...
              <span
                style="font-size: 10px; color: #666; text-align: center; word-break: break-all"
                >{{ iconName }}</span
              >
            </div>
          </div>
    ...
    <Variant title="所有图标（按字母顺序）">
      <div style="display: flex; gap: 12px; flex-wrap: wrap; align-items: center">
        ...
        >
          <TIcon :name="iconName" :size="state.size" :color="state.color" />
          <span style="font-size: 10px; color: #666; text-align: center; word-break: break-all">{{
            iconName
          }}</span>
        </div>
      </div>
