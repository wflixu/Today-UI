...
export const renderInput = (state: InputState, slots: InputSlots) => {
  ...
  if (contentAfterChildren.length > 0) {
    wrapperChildren.push(h('span', { class: state.contentAfter?.className }, contentAfterChildren));
  }

  // ========== Root Children ==========
  // Input Wrapper
  children.push(h('div', { class: state.inputWrapper?.className }, wrapperChildren));
